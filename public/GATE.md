...
### The center emblem (point of origin)
The big symbol in the gate's empty center is the **Earth POINT-OF-ORIGIN glyph** (glyph index
0, "Origin"): a tall up-arrow, a circle in the middle, a small down-arrowhead below. It is
**persistent** (shown whenever the gate is engaged) — NOT a dialing glyph that flies around.
- Style: bold cyan OUTLINE (stroke, hollow) with a glow — not filled solid.
- Size: fills a large part of the inner void. Void diameter = 2·0.66 = 1.32 world units; a
  glyph spans 100/256.71 = 0.39 world units at scale 1 → to fill ~75% of the void use
  scale ≈ 2.5.

### The circuit (routing topology)
From the clean reference (1781786061218_image.png):
- Each result box is connected on its **LEFT edge** by a trace that originates near the gate
  chevrons and steps down to that box's row (a short stepped path per box).
- On the **RIGHT** of the box column run **nested vertical lanes** (the return/bus) — one per
  box, nesting deeper for lower boxes, wrapping around the right side.
- A trace links the conductor up to the gate's **top chevron** (the network connects to the
  gate, it doesn't float in space).
- Per-box COLOR follows lock state: a locked box (and its traces, both the left connector and
  its right-side lane) turns **red**; unlocked stays cyan. So during dialing the circuit fills
  in red box-by-box from the top.
...
Red chevron lock = the chevron at that position turns red. In the clean reference the chevrons
read as **bold RED outlined arrowheads** pointing inward, sitting on the ring at each chevron
slot (hollow/thick, not small filled triangles).

Result boxes: number on the LEFT of each box; the box shows its dialed glyph as a **bold cyan
outline** (same style as the center emblem); box 7 is the point-of-origin. A locked box's
outline + connector turn red.

DESTINATION shows the address name (e.g. "ABYDOS") once chosen. The readout box shows the
current state line (e.g. "DIALING SEQUENCE ENGAGED"). Checklist rows show "OK" as each locks.
...
### Responsive rule
Designed at aspect **1.25:1** (1000×800); the cleanest UI reference (1781786061218) is
1.5:1 — the layout reads the same, panels just have more horizontal room. At that aspect the layout is pixel-faithful to the
